...
public sealed class GrantPermissionCommandHandler(
    ...
    : IRequestHandler<GrantPermissionCommand, Result>
{
    public async Task<Result> Handle(GrantPermissionCommand cmd, CancellationToken ct)
    {
        ...
        try
        {
            role.GrantPermission(permission, currentUser.UserId ?? "system", cmd.ExpiresAt);
        }
        catch (Domain.Exceptions.DomainException ex)
        {
            return Result.Failure(new Error("Permission.GrantFailed", ex.Message));
        }

        // Regenerate the role's snapshot — dedicated PermissionsSnapshot field,
        // replacing the legacy NHCS ConcurrencyStamp pattern.
        // The role aggregate already holds the new RolePermission in memory; we resolve
        // the codes by joining against the catalog.
