...
public sealed class ArchitectureTests
{
    ...
    public void CommandHandlers_ShouldEndWith_CommandHandler()
    {
        var result = Types.InAssembly(ApplicationAssembly)
            ...
            .Or()
            .HaveNameStartingWith("Update")
            .Or()
            .HaveNameStartingWith("Delete")
            .Or()
            .HaveNameStartingWith("Complete")
            .Should()
            .HaveNameEndingWith("Handler")
            .GetResult();

        result.IsSuccessful.Should().BeTrue(
            because: "Command handlers must end with 'Handler'.");
    }

    [Fact]
    public void DomainEntities_ShouldBeIn_EntitiesNamespace()
    {
        var result = Types.InAssembly(DomainAssembly)
            .That()
